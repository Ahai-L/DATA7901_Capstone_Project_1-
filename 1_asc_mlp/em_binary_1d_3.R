...
test_dat1 = asc_1d_em[-idx,]
write_rds(test_dat,"./1_asc_mlp/onehot_binary_1d/test_dat.rds")
# if for evaluation or predict run this code, then run 'data and tag', then 'import models'######################
test_dat = readRDS("./1_asc_mlp/em_binary_1d/test_dat.rds")
tr_dat =asc_1d_em[which(!as.numeric(rownames(asc_1d_em)) %in% as.numeric(rownames(test_dat))),] 

# data and tag ####################
tr_in_list = list(); test_in_list = list();j=1

for (i in svc_names) {
  tr_in_list[[j]] = as.matrix(tr_dat[,i])
  test_in_list[[j]] = as.matrix(test_dat[,i])
...
# so_ex input list
tr_ex_in_list = list(); test_ex_in_list = list(); j = 1
for (i in ex_names) {
  tr_ex_in_list[[j]] = as.matrix(tr_dat[,i])
  test_ex_in_list[[j]] = as.matrix(test_dat[,i])
  # tr_ex_in_list[[j]] = assign(paste(i,'tr',sep = "_"),as.matrix(tr_dat[,i]))
  # test_ex_in_list[[j]] = assign(paste(i,'test',sep = "_"),as.matrix(test_dat[,i]))
  j= j+1
}
...
svn_in = layer_input(shape(c(length(svn_names))),name = paste('snv_in',sep = '_'))

list_in = c(list_base_in,svn_in)
list_em = list_base_in
em_names = svc_names
# list_in = c(list_base_in,svn_in,list_ex_in)
# list_em = c(list_base_in,list_ex_in)
# em_names = c(svc_names,ex_names)

data_predict = as.data.frame(matrix(0, ncol = length(tag_names), nrow = nrow(test_dat)))
...
lists_in = rep(list(list()), length(tag_names)); list_model = list()
...
for (i in tag_names) {
  ...
  x_tr = switch(substr(i,1,4),
                'ASC_'= tr_in_list,
                'SUBO'= c(tr_in_list,tr_ex_in_list),
                'GREA'= c(tr_in_list,tr_ex_in_list),
                'SUBG'= c(tr_in_list,tr_ex_in_list)
  )
  x_test = switch(substr(i,1,4),
                  'ASC_'= test_in_list,
                  'SUBO'= c(test_in_list,test_ex_in_list),
                  'GREA'= c(test_in_list,test_ex_in_list),
                  'SUBG'= c(test_in_list,test_ex_in_list)
  )
  ...
  for (k in em_names) {
    ...
    lists_em_layer[[j]] = append(lists_em_layer[[j]],assign(paste(i,k,'1d',sep = '_'),
                                                            assign(paste(tag,k,'em',sep='_'),list_em[[l]] %>%
                                                                     layer_embedding(input_dim = vocabrulary+1,
                                                                                     output_dim = size,
                                                                                     mask_zero = T,
                                                                                     name = paste(tag,k,'em',sep='_'))) %>%
                                                              layer_reshape(c(size),name = paste(tag,k,'1d',sep = '_')))) 
    
    l = l+1
  }
  list_con_layer = c(list_con_layer,assign(paste(tag,'con',sep = '_'),
                                           layer_concatenate(c(lists_em_layer[[j]],svn_in),name = paste(tag,'con',sep = '_'))
  ))
  list_out <- c(list_out, assign(paste(tag,'out',sep = '_'),list_con_layer[[j]] %>%  
                                   layer_dense(units = 64,activation = 'relu',name = paste(tag,'dense',sep = '_')) %>%
                                   layer_dense(units = 1,activation = 'sigmoid',name = paste(tag,'out',sep = '_'))))
  lists_in[[j]] = c(lists_in[[j]],list_in)
  list_model = c(list_model,keras_model(lists_in[[j]],list_out[[j]]))
  ...
  list_model[[j]] %>% save_model_hdf5(paste('./1_asc_mlp/em_binary_1d/model_',tag,'.h5',sep = ''))
...
write_rds(data_predict,"./1_asc_mlp/em_binary_1d/data_predict.rds")
write_rds(list_model,"./1_asc_mlp/em_binary_1d/list_model.rds")
data_predict = readRDS("./1_asc_mlp/em_binary_1d/data_predict.rds")
# predict update ###################
# dat: the predict result data frame, such as data_predict
#out put : updated predict result only related value will remain
predict_update <- function(dat, threshold = 0.1) {
  # the list ######################
...
ord_predict = data_predict[,ord_names]
so_predict = data_predict[,so_names]
gg_predict = data_predict[,gg_names]
sg_predict = data_predict[,sg_names]
# pred: a single data, 
...
ord_evaluate = asc_evaluation_single(ord_predict,test_tag[])
...
ord_predict_h = top1_top2(data_predict[,ord_names],test_tag[ord_names])
so_predict_h = top1_top2(data_predict[,so_names],test_tag[so_names])
gg_predict_h = top1_top2(data_predict[,gg_names],test_tag[gg_names])
sg_predict_h = top1_top2(data_predict[,sg_names],test_tag[sg_names])
