...
for (j in svc_names) {
  vocabrulary = length(unique(asc_1d_em[[j]]))
  size = min(50,(round(0.5*vocabrulary)+1))
  list_base_in = append(list_base_in, assign(paste(j,'base_in',sep = '_'),
                                             layer_input(shape(c(1)),name = paste(j,'base_in',sep = '_'))))
  list_em =  append (list_em, list_base_in[[j]] %>% 
                       layer_embedding(input_dim = vocabrulary+1,
                                                   output_dim = size,
                                                   mask_zero = T,
                                                   name = paste(j,'em',sep='_'))%>%
                       layer_reshape(c(size),name = paste(j,'1d',sep = '_')))
...
for (j in ex_names) {
  vocabrulary = length(unique(asc_1d_em[[j]]))
  size = min(50,(round(0.5*vocabrulary)+1))
  list_ex_in = append(list_ex_in, assign(paste(j,'ex_in',sep = '_'),
                                         layer_input(shape(c(1)),name = paste(j,'ex_in',sep = '_'))))
  list_ex_em = append(list_ex_em, list_ex_in[[j]] %>%
                        layer_embedding(input_dim = vocabrulary+1,
                                        output_dim = size,
                                        mask_zero = T,
                                        name = paste(j,'em',sep='_'))%>%
