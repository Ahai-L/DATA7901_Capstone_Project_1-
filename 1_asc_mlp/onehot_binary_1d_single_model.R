...
library(crayon)
library(readr)
asc_1d_onehot_4 = readRDS('./0_general/asc_1d_onehot_0.rds')
# index list#######################
...
write_rds(test_dat,"./1_asc_mlp/onehot_binary_1d_single_model/test_dat.rds")
# if for evaluation or predict run this code, then go to import models section######################
test_dat = readRDS("./1_asc_mlp/onehot_binary_1d_single_model/test_dat.rds")
...
resampling_sub <- function(x,feature,s1=1,s2=5000) {
...
tr_dat_n = resampling_sub(tr_dat,"ASC_ORD")
tr_in_base = as.matrix(tr_dat_n[,-which(
  names(tr_dat) %in% c('PROJECT_CODE','SITE_ID',ex_names,ord_names,so_names,gg_names,sg_names))])
tr_in_ex = as.matrix(tr_dat_n[,ex_names])

test_in_base = as.matrix(test_dat[,-which(
  names(test_dat) %in% c('PROJECT_CODE','SITE_ID',ex_names,ord_names,so_names,gg_names,sg_names))])
test_in_ex = as.matrix(test_dat[,ex_names])

tr_tag = tr_dat_n[,tag_names]
test_tag = test_dat[,tag_names]
# data for matlab ######################
dat_asc_test = test_dat[,-which(
  names(test_dat) %in% c('PROJECT_CODE','SITE_ID',ex_names,ord_names,so_names,gg_names,sg_names))]
dat_asc_n = rbind(tr_dat_n,test_dat)
dat_asc = tr_dat_n[,-which(
  names(tr_dat_n) %in% c('PROJECT_CODE','SITE_ID',ex_names,ord_names,so_names,gg_names,sg_names))]
tag_ord = dat_asc_n[,ord_names]
write.csv(dat_asc,'./1_asc_mlp/matlab_file/dat_asc.csv',row.names = F)
write.csv(dat_asc_test,'./1_asc_mlp/matlab_file/dat_asc_test.csv',row.names = F)
write.csv(tag_ord,'./1_asc_mlp/matlab_file/tag_ord.csv',row.names = F)

# single model####################
single_model <- function(input,tag,tag_name,test) {
  base_in = layer_input(shape(c(ncol(input))),name = paste('base_in',tag_name,sep = '_'))
  output = base_in %>%
    layer_dense(units = 128, activation = 'relu', name = paste('dense_1',tag_name, sep = '_')) %>%
    layer_dense(units = 64, activation = 'relu', name = paste('dense_2',tag_name, sep = '_')) %>%
    layer_dense(units = 1, activation = 'sigmoid', name = paste('output',tag_name, sep = '_'))
  model = keras_model(base_in,output)
  model %>% compile(
    optimizer=optimizer_rmsprop(0.0001),
    #loss = loss_binary_crossentropy,
    loss = "binary_crossentropy",#"sparse_categorical_crossentropy",
    metrics = metric_binary_accuracy#metric_sparse_categorical_crossentropy 
  )
  model %>% fit(
    x = input,
    y = tag[[tag_name]],
    ...
    validation_data = list(test_in_base,test_tag[[tag_name]])
  )
...
if (idx_tag==1) {
  test_tag_n = test_tag
  predict = single_model(tr_in_base,
               tr_tag,
               ord_names[idx_tag],test_in_base)
} else if (idx_tag==2) {
  test_tag_n = test_tag[(idx_tag-1)!=1,]
  predict = single_model(tr_in_base[tr_tag[,(idx_tag-1)]!=1,],
                         tr_tag[tr_tag[,(idx_tag-1)]!=1,],
                         ord_names[idx_tag],
                         test_in_base[test_tag[,(idx_tag-1)]!=1,])
} else {
  test_tag_n = test_tag[rowSums(test_tag[,1:(idx_tag-1)])!=1,]
  predict = single_model(tr_in_base[rowSums(tr_tag[,1:(idx_tag-1)])!=1,],
                         tr_tag[rowSums(tr_tag[,1:(idx_tag-1)])!=1,],
                         ord_names[idx_tag],
                         test_in_base[rowSums(test_tag[,1:(idx_tag-1)])!=1,])
}



#  single_model(tr_in_base[rowSums(test_tag[,1:2])!=1,],tr_tag[tr_tag$ASC_ORD_AN!=1,],'ASC_ORD_OR',test_in_base)
ord_predict = predict
#colnames(an_predict)[1]=names(test_tag['ASC_ORD_AN'])
ord_predict[ord_predict>0.4]=1
ord_predict[ord_predict!=1]=0
confusionMatrix(factor(ord_predict),factor(test_tag_n[[idx_tag]]))
...
for (i in tag_names) {
  ...
  list_model[[j]] %>% save_model_hdf5(paste('./1_asc_mlp/onehot_binary_1d_single_model/model_',tag,'.h5',sep = ''))
...
for (i in tag_names) {
  ...
  path = "./1_asc_mlp/onehot_binary_1d_single_model/"
...
write_rds(ord_confusion,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_ord.rds")
write_rds(ord_confusion_top2,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_ord_top2.rds")
write_rds(so_confusion,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_so.rds")
write_rds(so_confusion_top2,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_so_top2.rds")
write_rds(gg_confusion,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_gg.rds")
write_rds(gg_confusion_top2,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_gg_top2.rds")
write_rds(sg_confusion,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_sg.rds")
write_rds(sg_confusion_top2,"./1_asc_mlp/onehot_binary_1d_single_model/onehot_binary_1d_single_model_confusion_h_sg_top2.rds")
# evaluation demos ##########################
