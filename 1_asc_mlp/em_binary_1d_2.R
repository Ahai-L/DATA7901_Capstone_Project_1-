...
for (i in tag_names) {
  ...
  list_model = c(list_model,keras_model(lists_in[[j]],list_out))
...
test_out_list = c(asplit(test_tag,2),asplit(mlp_so_tag_test,2),
...
# single level acc#################
# ord###################
for (i in 1:length(test_tag)) {
  colnames(asc_predict[[i]]) = colnames(test_tag[i])
  rownames(asc_predict[[i]]) = rownames(test_tag[i])
}
ord_predict = as.data.frame(abind(asc_predict[1:length(test_tag)],along=2))
...
ord_predict = cbind(colnames(ord_predict)[apply(ord_predict,1,function(x) match(sort(x[x>0],decreasing = T)[1],x))],
                    colnames(ord_predict)[apply(ord_predict,1,function(x) ifelse(is.na(match(sort(x[x>0],decreasing = T)[2],x)),
                     ...
                     colnames(test_tag)[apply(test_tag,1,which.max)],
                     ord_predict)
...
for (i in 1:length(mlp_so_tag_test)) {
  colnames(asc_predict[[i+length(test_tag)]]) = colnames(mlp_so_tag_test[i])
  rownames(asc_predict[[i+length(test_tag)]]) = rownames(mlp_so_tag_test[i])
}
so_predict = as.data.frame(abind(asc_predict[(length(test_tag)+1):
                                               (length(test_tag)+length(mlp_so_tag_test))],along=2))
...
for (i in 1:length(mlp_gg_tag_test)) {
  colnames(asc_predict[[i+length(test_tag)+length(mlp_so_tag_test)]]) = colnames(mlp_gg_tag_test[i])
  rownames(asc_predict[[i+length(test_tag)+length(mlp_so_tag_test)]]) = rownames(mlp_gg_tag_test[i])
}
gg_predict = as.data.frame(abind(asc_predict[(length(test_tag)+length(mlp_so_tag_test)+1):
                                               (length(test_tag)+length(mlp_gg_tag_test)+
...
for (i in 1:length(mlp_sg_tag_test)) {
  colnames(asc_predict[[i+length(test_tag)+length(mlp_so_tag_test)+length(mlp_gg_tag_test)]]) = colnames(mlp_sg_tag_test[i])
  rownames(asc_predict[[i+length(test_tag)+length(mlp_so_tag_test)+length(mlp_gg_tag_test)]]) = rownames(mlp_sg_tag_test[i])
}
sg_predict = as.data.frame(abind(asc_predict[(length(test_tag)+length(mlp_so_tag_test)+length(mlp_gg_tag_test)+1):
                                               (length(test_tag)+length(mlp_gg_tag_test)+
...
# one by one#####################
ord_an = data.frame('real'= test_tag[,1], 'predict'= asc_predict_original[[1]])
