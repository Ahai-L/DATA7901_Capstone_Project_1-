...
mlp_names = list("ASC"=mlp_asc_names,'SO'=mlp_so_names,'GG'=mlp_gg_names,'SG'=mlp_sg_names);
extra_in_list = data.frame('INDEX'=c(2),'EXTRA_IN'=c(length(mlp_so_ex)))
j=1;tlist_in = list(); list_in = list(); list_out = list();
for (i in names(mlp_names)) {
  if (j==1) {
  ...
  list_out <- c(list_out, assign(paste(i,'out',sep = '_'),tlist_in[[j]] %>%  
                                   layer_dense(units = 128,activation = 'relu',name = paste(i,'dense_128',sep = '_')) %>%
                                   layer_dense(units = 64,activation = 'relu',name = paste(i,'dense_64',sep = '_')) %>%
                                   layer_dense(units = length(mlp_names[[i]]),activation = 'softmax',name = paste(i,'out',sep = '_'))))
...
model %>% compile(
  ...
  loss = rep("categorical_crossentropy",length(mlp_names)),
  metrics = metric_top_1_categorical_accuracy
)
model
model %>% save_model_hdf5('./1_asc_mlp/mlp_asc_softmax.h5')

# fitting################
tr_in_list = list(as.matrix(mlp_asc_in_tr),as.matrix(mlp_soex_tr))
tr_out_list = list(as.matrix(mlp_asc_tag_tr),as.matrix(mlp_so_tag_tr),
                   as.matrix(mlp_gg_tag_tr),as.matrix(mlp_sg_tag_tr))
...
test_out_list = list(as.matrix(mlp_asc_tag_test),as.matrix(mlp_so_tag_test),
                   as.matrix(mlp_gg_tag_test),as.matrix(mlp_sg_tag_test))

result = model %>% evaluate(test_in_list,test_out_list)
asc_predict <- model %>% predict(test_in_list)

# confusion matrix####################
colnames(asc_predict[[1]]) = colnames(mlp_asc_tag_test)
rownames(asc_predict[[1]]) = rownames(mlp_asc_tag_test)
colnames(asc_predict[[2]]) = colnames(mlp_so_tag_test)
rownames(asc_predict[[2]]) = rownames(mlp_so_tag_test)
colnames(asc_predict[[3]]) = colnames(mlp_gg_tag_test)
rownames(asc_predict[[3]]) = rownames(mlp_gg_tag_test)
colnames(asc_predict[[4]]) = colnames(mlp_sg_tag_test)
rownames(asc_predict[[4]]) = rownames(mlp_sg_tag_test)
ord_predict = as.data.frame(asc_predict[[1]])
so_predict = as.data.frame(asc_predict[[2]])
gg_predict = as.data.frame(asc_predict[[3]])
sg_predict = as.data.frame(asc_predict[[4]])
