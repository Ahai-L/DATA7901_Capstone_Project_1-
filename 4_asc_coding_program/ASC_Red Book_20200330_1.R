...
hy_p <- filter(hy_nnull,ELEM_TYPE_CODE %in% c('TDF','ITF','LAG'))
...
ku_nnull <- hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
  ...
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  lefe_join(.,fts,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO')) %>%
...
ku_11 <- filter(ku_nnull,BOUND_DISTINCT %in% c('S','A','C'))# %>%
...
ku_2 <- ku_nnull[which(grepl("B2",ku_nnull$HORIZON_NAME) & (ku_nnull$VALUE_4A1 < 5.5| ku_nnull$VALUE_PH <5.5)),]
