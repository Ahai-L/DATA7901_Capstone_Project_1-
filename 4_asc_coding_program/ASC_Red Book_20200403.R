...
or_nnull <- hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
                     'HORIZON_NAME','UPPER_DEPTH','LOWER_DEPTH')] %>%
  ...
  anti_join(.,.[which(.$HORIZON_NAME == '-' | is.na(.$HORIZON_NAME)|
                      .$DESIGN_MASTER == '-'|is.na(.$DESIGN_MASTER)|
                        .$ELEM_TYPE_CODE == '-'|is.na(.$ELEM_TYPE_CODE)),],by = c('PROJECT_CODE','SITE_ID')) %>%
  filter(.,!ASC_ORD %in% c('AN'))
# or_nnull$CAR_CLAY <- 6/55*or_nnull$VALUE_2Z2_Clay+11+5/11
...
# %  of carbon / % of caly ratio satisfied subset
or_2a <- filter(or_nnull,!is.na(or_nnull$VALUE_6B_6A),!is.na(or_nnull$VALUE_2Z2_Clay)) %>%
  filter(., VALUE_6B_6A >= (6/55*VALUE_2Z2_Clay + 11 + 5/11))
# %  of carbon / % of caly ratio unsatisfied subset
or_2b <- filter(or_nnull,!is.na(or_nnull$VALUE_6B_6A),!is.na(or_nnull$VALUE_2Z2_Clay)) %>%
  filter(., VALUE_6B_6A < (6/55*VALUE_2Z2_Clay + 11 + 5/11))
# 'O' OR 'P' horizon 
or_2c <- or_nnull[grepl('O1|O|O2|P|P1|P2',or_nnull$HORIZON_NAME),]
#or_2cc <- or_nnull[grepl('O|P',or_nnull$DESIGN_MASTER),]
#unique(or_nnull[grepl('O1|O|O2|P|P1|P2',or_nnull$HORIZON_NAME),'HORIZON_NAME'])
# (or_2a or or_2c) and (not or_2b), and UPPER_DEPTH within 0.8m
or_2 <- rbind(or_2c,or_2a) %>%
  anti_join(.,or_2b,by = c('PROJECT_CODE','SITE_ID')) %>%
  inner_join(.,filter(or_nnull, UPPER_DEPTH <= 0.8))
...
# or_3: Have organic materials extending from the surface to a minimum depth of 0.1 m
#surface horizon with "O" or "p" design_master
or_31 <- filter(or_nnull,DESIGN_MASTER %in% c('O','P'),HORIZON_NO == 1)
#accumulate the next horizon with "O" or "p" design_master
or_join <- semi_join(filter(or_nnull,DESIGN_MASTER %in% c('O','P'),HORIZON_NO == 2),or_31,
                      by = c('PROJECT_CODE','SITE_ID')) 
# or_join3 <- semi_join(filter(or_nnull,DESIGN_MASTER %in% c('O','P'),HORIZON_NO == 3),or_join,
#                      by = c('PROJECT_CODE','SITE_ID')) 
or_31 <- rbind(or_31,or_join)
i <- 2
j <- nrow(or_join)
while (j>0) {
  i = i+1
  or_join <- semi_join(filter(or_nnull,DESIGN_MASTER %in% c('O','P'),HORIZON_NO == i),or_join,
                       by = c('PROJECT_CODE','SITE_ID')) 
  or_31 <- rbind(or_31,or_join)
  j <- nrow(or_join)
}
#calculate  horizon thickness 
or_31$THICKNESS <- or_31$LOWER_DEPTH -or_31$UPPER_DEPTH
#sum up the thickness of horizons for same profile, filter out  sum thickness <0.1
or_32<-aggregate(THICKNESS ~ PROJECT_CODE+SITE_ID, data = or_31,FUN = sum) %>%
  filter(., THICKNESS>=0.1)
#extract the max horizon_no for each solum, full join with or_32
or_31 <- aggregate(HORIZON_NO ~ PROJECT_CODE+SITE_ID, data = or_31, FUN = max) %>%
  full_join(.,or_32)

# CHECK THE UNDERLAY HORIZON TO BE 'R' 'B2m' , or 'C'
or_33 <- sqldf('select B.*, A.HORIZON_NO HOR_NO_A, A.DESIGN_MASTER,A.HORIZON_NAME
      from or_nnull A,or_31 B
      where A.PROJECT_CODE = B.PROJECT_CODE and A.SITE_ID = B.SITE_ID and
      A.HORIZON_NO = B.HORIZON_NO+1 and 
              (A.DESIGN_MASTER = "R" OR
              A.HORIZON_NAME LIKE "%B2m%" 
              )') 
#CHECK IF THE HORIZON_NO is the last horizon in the solum
or_nnull_max <- aggregate(HORIZON_NO ~ PROJECT_CODE+SITE_ID, data = or_nnull, FUN = max) 
or_34 <- merge(or_31,or_nnull_max,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO'))
or_3 <- full_join(or_33,or_34)



# or_nnull[which(or_nnull$PROJECT_CODE =='SWRES'& or_nnull$SITE_ID == 371),]             
...
#add a column to or
or_p$ASC_ORD <- rep("OR",nrow(or_p))
names(or_p)[8] <- 'ASC_ORD_P'
all_p <- or_p
# Confusion matrix
...
or_fp_fn_hor <- left_join(or_fp_fn,hor[,c('PROJECT_CODE','SITE_ID','HORIZON_NO','DESIGN_MASTER','HORIZON_NAME',
...
po_nnull <- anti_join(hor[,c('PROJECT_CODE','SITE_ID','HORIZON_NO','DESIGN_MASTER','HORIZON_NAME')],
                      ...
                      by= c('PROJECT_CODE','SITE_ID')) %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  anti_join(.,filter(all_p,ASC_ORD_P !='PO' ),by= c('PROJECT_CODE','SITE_ID'))%>%
  filter(.,!ASC_ORD %in% c('AN','OR'))
sum(is.na(po_nnull))
# dataset with null value in attributes for judging OR, and is labeled with OR
...
po_p <- po_p[which(grepl("^(1B|B)+(.*)(h|s)(.*)",po_p$HORIZON_NAME)),]
unique(po_p$HORIZON_NAME)
# po_p <- po_p[which(grepl("+h|+s|+hs",po_p$HORIZON_NAME)),] 
#add a column to or
po_p$ASC_ORD <- rep("PO",nrow(po_p))
names(po_p)[6] <- 'ASC_ORD_P'
...
po_vat <- po_nnull %>%
  ...
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
# unique check
nrow(unique(po_vat[,1:2])) == nrow(unique(po_vat[,1:3]))
#join the predict data
po_vat <- left_join(po_vat,po_p,by = c('PROJECT_CODE','SITE_ID'))
# mark other orders except "PO" in ASC_ORD with "OTHERS",
po_vat[which(!is.na(po_vat$ASC_ORD) & po_vat$ASC_ORD != 'PO'),'ASC_ORD'] = 'OTHERS'
# NA in asc_ord as "N/A"
po_vat[is.na(po_vat)]<-"N/A"
# value in ASC_ORD_P except for 'PO', and is "OTHERS" in ASC_ORD, mark as "OTHERS" as well
po_vat[which(po_vat$ASC_ORD == "OTHERS" & po_vat$ASC_ORD_P != 'PO'),'ASC_ORD_P'] = 'OTHERS'
# feed the data to confusionmatrix
po_confmat <- confusionMatrix(as.factor(po_vat$ASC_ORD_P),reference = as.factor(po_vat$ASC_ORD))
print(po_confmat)
...
ve_nnull_1 <- hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
                   'HORIZON_NAME','UPPER_DEPTH','LOWER_DEPTH','TEXTURE_CODE','SOIL_WATER_STAT')] %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  left_join(.,osc[,c('PROJECT_CODE','SITE_ID','OBS_NO','STATUS')],
            by = c('PROJECT_CODE','SITE_ID','OBS_NO') ) %>%
  ...
  left_join(.,hst[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','STRUCT_NO','PEDALITY_TYPE')],
            by = c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO')) %>%
  left_join(.,lab_2z2_clay,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO')) %>%
  anti_join(.,.[which(.$STATUS == '-' | is.na(.$STATUS)|
                      .$TEXTURE_CODE == '-'|is.na(.$TEXTURE_CODE)),],by = c('PROJECT_CODE','SITE_ID')) %>%
  anti_join(.,filter(all_p,ASC_ORD_P !='VE' ),by= c('PROJECT_CODE','SITE_ID'))%>%
  filter(.,!ASC_ORD %in% c('AN','OR','PO'))
# ve_11 : horizons in the surface with crusty horizon and 0.03m or less thick LEVEL:HOR
ve_11 <- filter(ve_nnull_1, (HORIZON_NO == 1|UPPER_DEPTH <=0), LOWER_DEPTH-UPPER_DEPTH <=0.03,STATUS == 'C')
# horizons in the surface and have 35% or more clay

ve_12 <- filter(ve_nnull_1,(HORIZON_NO == 1|UPPER_DEPTH <=0),VALUE_2Z2_Clay >=35) %>%
  full_join(.,filter(ve_nnull_1,HORIZON_NO == 1,grepl('LC|LMC|MC|MHC|HC',ve_nnull_1$TEXTURE_CODE)))
# ve_12 <- filter(ve_nnull,HORIZON_NO == 1,TEXTURE_CODE %in% c('LC', 'LMC','MC','MHC','HC','KSLC','C'))
# All Solums except surface horizon, with a clay field texture or 35% or more clay
ve_13 <- filter(ve_nnull_1, HORIZON_NO != 1 ) %>%
  group_by(PROJECT_CODE,SITE_ID) %>%
  filter(all(VALUE_2Z2_Clay >=35 | TEXTURE_CODE %in% c('LC', 'LMC','MC','MHC','HC','KSLC'))) 
# ve_13 <- filter(ve_nnull, HORIZON_NO != 1 ) %>%
#   group_by(PROJECT_CODE,SITE_ID) %>%
#   filter(all(TEXTURE_CODE %in% c('LC', 'LMC','MC','MHC','HC','KLC','C'))) %>%
#   distinct(PROJECT_CODE,SITE_ID)
# 
ve_1 <- full_join(ve_11,ve_12,) %>%
  inner_join(.,ve_13,by = c('PROJECT_CODE','SITE_ID')) %>%
  distinct(PROJECT_CODE,SITE_ID)

#ve_2 
ve_nnull_2 <- hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
                   'HORIZON_NAME','UPPER_DEPTH','LOWER_DEPTH','TEXTURE_CODE','SOIL_WATER_STAT')] %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  left_join(.,osc[,c('PROJECT_CODE','SITE_ID','OBS_NO','STATUS')],
            by = c('PROJECT_CODE','SITE_ID','OBS_NO') ) %>%
...
# when dry
ve_21 <- ve_nnull_2[which(!ve_nnull_2$SOIL_WATER_STAT %in% c('M','W')),]
...
ve_22 <- full_join(ve_nnull_2[which(ve_nnull_2$STATUS %in% c('G','M','C')),], 
                   ve_nnull_2[which(grepl("ap|p",ve_nnull_2$HORIZON_NAME) & ve_nnull_2$DESIGN_MASTER == 'A'),],)
                  # by = c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO')) 
...
ve_nnull_3 <- hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
                   'HORIZON_NAME','UPPER_DEPTH','LOWER_DEPTH','TEXTURE_CODE','SOIL_WATER_STAT')] %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  left_join(.,osc[,c('PROJECT_CODE','SITE_ID','OBS_NO','STATUS')],
            by = c('PROJECT_CODE','SITE_ID','OBS_NO') ) %>%
  ...
  left_join(.,hst[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','STRUCT_NO','PEDALITY_TYPE')],
            by = c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO')) %>%
  left_join(.,lab_2z2_clay,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO')) %>%
  anti_join(.,.[which(  .$CUTAN_TYPE == '-' | is.na(.$CUTAN_TYPE)|
                        .$PEDALITY_TYPE == '-' | is.na(.$PEDALITY_TYPE)
                         ),],by = c('PROJECT_CODE','SITE_ID'))%>%
  anti_join(.,filter(all_p,ASC_ORD_P !='VE' ),by= c('PROJECT_CODE','SITE_ID'))%>%
  filter(.,!ASC_ORD %in% c('AN','OR','PO'))
...
ve_3 <- ve_nnull_3[which(ve_nnull_3$CUTAN_TYPE == 'K' | ve_nnull_3$PEDALITY_TYPE == 'LE'),]
nrow(unique(ve_nnull_3[,1:2]))
...
#add a column to or
ve_p$ASC_ORD <- rep("VE",nrow(ve_p))
names(ve_p)[11] <- 'ASC_ORD_P'

all_p <- full_join(all_p,ve_p)


# Confusion matrix
ve_nnull <- inner_join(ve_nnull_1,ve_nnull_2) %>% inner_join(.,ve_nnull_3)
# dataset with null value in attributes for judging OR, and is labeled with OR
...
hy_nnull <- sit[,c('PROJECT_CODE','SITE_ID',"ELEM_TYPE_CODE")] %>%
  left_join(.,obs[,c('PROJECT_CODE','SITE_ID','DRAINAGE')],
            by = c('PROJECT_CODE','SITE_ID') ) %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  anti_join(.,.[which(.$ELEM_TYPE_CODE == '-' | is.na(.$ELEM_TYPE_CODE)|
                        .$DRAINAGE == '-'|is.na(.$DRAINAGE)),],
            by = c('PROJECT_CODE','SITE_ID'))%>%
  anti_join(.,filter(all_p,ASC_ORD_P !='HY' ),by= c('PROJECT_CODE','SITE_ID'))%>%
  filter(.,!ASC_ORD %in% c('AN','OR','PO','VE'))

# 
hy_p <- hy_nnull[which(hy_nnull$ELEM_TYPE_CODE %in% c('TDF','ITF','LAG','STF','SWP','LAK') | 
                         hy_nnull$DRAINAGE == 1),]
# # if elem_type_code in TDF,ITF, LAG, THEN IS 'HY'
# hy_p <- filter(hy_nnull,ELEM_TYPE_CODE %in% c('TDF','ITF','LAG','STF','SWP','LAK'))
# # elem_type_code in 'STF','SWP','LAK', and drainage == 1
# hy_p <- rbind(hy_p, hy_nnull[which(hy_nnull$ELEM_TYPE_CODE %in% c('STF','SWP','LAK')|hy_nnull$DRAINAGE == 1),])

#hy_p <- rbind(hy_p, filter(hy_nnull,ELEM_TYPE_CODE %in% c('STF','SWP','LAK'),DRAINAGE == 1))
#find solum with ELEM_TYPE_CODE means 'wet'
#hy_11 <- filter(hy_nnull, ELEM_TYPE_CODE %in% c('TDF','ITF','STF','SWP','LAG','LAK')) 
...
#add a column to or
hy_p$ASC_ORD <- rep("HY",nrow(hy_p))
names(hy_p)[5] <- 'ASC_ORD_P'
all_p <- full_join(all_p,hy_p)


# Confusion matrix

# dataset with null value in attributes for judging OR, and is labeled with OR
hy_null <- filter(ocl[,c(1,2,7)], ASC_ORD == 'HY') %>%
  anti_join(.,hy_nnull,by = c('PROJECT_CODE','SITE_ID')) %>%
  left_join(.,sit[,c('PROJECT_CODE','SITE_ID',"ELEM_TYPE_CODE")]) %>%
  left_join(.,obs[,c('PROJECT_CODE','SITE_ID','DRAINAGE')],
            by = c('PROJECT_CODE','SITE_ID') ) 

# hy_nnull distinct with project_code and site_id, and add asc_ord from ocl
hy_vat <- hy_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
# unique check
nrow(unique(hy_vat[,1:2])) == nrow(unique(hy_vat[,1:3]))
#join the predict data
hy_vat <- left_join(hy_vat,hy_p,by = c('PROJECT_CODE','SITE_ID'))
# mark other orders except "hy" in ASC_ORD with "OTHERS",
hy_vat[which(!is.na(hy_vat$ASC_ORD) & hy_vat$ASC_ORD != 'HY'),'ASC_ORD'] = 'OTHERS'
# NA in asc_ord as "N/A"
hy_vat[is.na(hy_vat)]<-"N/A"
# value in ASC_ORD_P except for 'hy', and is "OTHERS" in ASC_ORD, mark as "OTHERS" as well
hy_vat[which(hy_vat$ASC_ORD == "OTHERS" & hy_vat$ASC_ORD_P != 'HY'),'ASC_ORD_P'] = 'OTHERS'
# feed the data to confusionmatrix
hy_confmat <- confusionMatrix(as.factor(hy_vat$ASC_ORD_P),reference = as.factor(hy_vat$ASC_ORD))
print(hy_confmat)

# similar as abohy but tick out the "N/A" values in ASC_ORD
hy_vat_1 <- hy_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
hy_vat_1 <- hy_vat_1[which(!is.na(hy_vat_1$ASC_ORD)),]
nrow(unique(hy_vat_1[,1:2])) == nrow(unique(hy_vat_1[,1:3]))
hy_vat_1 <- left_join(hy_vat_1,hy_p,by = c('PROJECT_CODE','SITE_ID'))
hy_vat_1[which(hy_vat_1$ASC_ORD != 'HY'),'ASC_ORD'] = 'OTHERS'
hy_vat_1[is.na(hy_vat_1)]<-"OTHERS"
hy_confmat_1 <- confusionMatrix(as.factor(hy_vat_1$ASC_ORD_P),reference = as.factor(hy_vat_1$ASC_ORD))
print(hy_confmat_1)



# write to files
hy_tp <- filter(hy_vat_1,ASC_ORD == 'HY',ASC_ORD_P == 'HY') 
hy_tp_hor <- left_join(hy_tp,hy_nnull, by = c('PROJECT_CODE','SITE_ID') )
hy_fp_fn <- filter(hy_vat_1,ASC_ORD != ASC_ORD_P) 
hy_fp_fn <- sqldf('select A.PROJECT_CODE, A.SITE_ID, B.ASC_ORD, A.ASC_ORD_P
        FROM hy_fp_fn A, ocl B
        WHERE A.PROJECT_CODE = B.PROJECT_CODE AND
        A.SITE_ID = B.SITE_ID')
hy_fp_fn[which(is.na(hy_fp_fn)),]  

hy_fp_fn_hor <- left_join(hy_fp_fn,hy_nnull)


# hy_fp_fn_hor$hy_1 <- ifelse(is.na(match(paste0(hy_fp_fn_hor$PROJECT_CODE,hy_fp_fn_hor$SITE_ID),
#                                          paste0(hy_1$PROJECT_CODE,hy_1$SITE_ID))),"NO","YES")
# hy_fp_fn_hor$hy_p <- ifelse(is.na(match(paste0(hy_fp_fn_hor$PROJECT_CODE,hy_fp_fn_hor$SITE_ID),
#                                          paste0(hy_p$PROJECT_CODE,hy_p$SITE_ID))),"NO","YES")

# hy_fp_fn_hor[which(is.na(hy_fp_fn_hor)),]

hy_predict <- filter(hy_vat,ASC_ORD =="N/A",ASC_ORD_P == 'HY' )

write.csv(hy_fp_fn, file =paste("C:/Users/lzccn/Downloads/des/files/hy_fp_fn",
                                format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
write.csv(hy_fp_fn_hor, file =paste("C:/Users/lzccn/Downloads/des/files/hy_fp_fn_hor",
                                    format(Sys.time(),"%d-%b-%Y %H.%M"),".csv",sep = " "))
write.csv(hy_predict, file =paste("C:/Users/lzccn/Downloads/des/files/hy_predict",
                                  format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
write.csv(hy_null, file =paste("C:/Users/lzccn/Downloads/des/files/hy_null",
                               format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
...
# ASC-KU- Kurosols ########################################
# Soils other than Hydrosols with a clear or abrupt textural B horizon and in
# which the major part of the upper 0.2 m of the B2 horizon (or the major part
# of the entire B2 horizon if it is less than 0.2 m thick) is strongly acid.

# data, only 1347 qulified soil site data 

ku_nnull <- hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
                     'HORIZON_NAME','TEXTURE_CODE','BOUND_DISTINCT')] %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  ...
  anti_join(.,.[which(.$DESIGN_MASTER == '-' | is.na(.$DESIGN_MASTER)|
            ...
            by = c('PROJECT_CODE','SITE_ID')) %>%
  anti_join(.,filter(all_p,ASC_ORD_P !='KU' ),by= c('PROJECT_CODE','SITE_ID'))%>%
  filter(.,!ASC_ORD %in% c('AN','OR','PO','VE','HY'))
nrow(unique(ku_nnull[which(ku_nnull$ASC_ORD == 'KU'),1:2]))

# clear or abrupt textural horizons in 'B' HORIZON
#ku_11 <- filter(ku_nnull, DESIGN_MASTER == 'B')

ku_1a <- sqldf('select A.PROJECT_CODE, A.SITE_ID, A.HORIZON_NO AS A_HOR, B.HORIZON_NO AS B_HOR,
      A.HORIZON_NAME AS A_HOR_NAME, B.HORIZON_NAME AS B_HOR_NAME, A.BOUND_DISTINCT AS A_BOUND, A.TEXTURE_CODE AS A_TEX,
      B.TEXTURE_CODE AS B_TEX, A.ASC_ORD, A.VALUE_2Z2_Clay as A_CLAY, B.VALUE_2Z2_Clay as B_CLAY
      from ku_nnull A,ku_nnull B
      where A.PROJECT_CODE = B.PROJECT_CODE and A.SITE_ID = B.SITE_ID and
      A.HORIZON_NO = B.HORIZON_NO-1  and A.BOUND_DISTINCT IN ("A","C","S") and
      B.DESIGN_MASTER = "B"
      and B.VALUE_2Z2_Clay < 2*A.VALUE_2Z2_Clay ') 
  
ku_1 <- sqldf('select A.PROJECT_CODE, A.SITE_ID, A.HORIZON_NO AS A_HOR, B.HORIZON_NO AS B_HOR,
      A.HORIZON_NAME AS A_HOR_NAME, B.HORIZON_NAME AS B_HOR_NAME, A.BOUND_DISTINCT AS A_BOUND, A.TEXTURE_CODE AS A_TEX,
      B.TEXTURE_CODE AS B_TEX, A.ASC_ORD, A.VALUE_2Z2_Clay as A_CLAY, B.VALUE_2Z2_Clay as B_CLAY
      from ku_nnull A,ku_nnull B
      where A.PROJECT_CODE = B.PROJECT_CODE and A.SITE_ID = B.SITE_ID and
      ...
      B.DESIGN_MASTER = "B"
      and (
            (A.TEXTURE_CODE IN("S","LS","FS","LFS","KS","LKS", "CS","CKS","CFS") and
            B.TEXTURE_CODE in ("LFSY","L","ZL","SCL","CL","CLS","ZCL","LC","ZLC","LMC","MC","MHC","HC",
            "KSL", "KSCL","CLKS","KSLC","KSLMC","KSMC","KSMHC","KSHC", "SLC","SLMC","SMC","SMHC","SHC","FSL",
            "FSCL","CLFS","FSLC","FSLMC","FSMC","FSMHC","FSHC") OR
            A.TEXTURE_CODE IN ("SL","KSL","FSL") and
            B.TEXTURE_CODE in ("CL","CLS","ZCL","LC","ZLC","LMC","MC","MHC","HC","CLKS","KSLC","KSLMC","KSMC",
            "KSMHC","KSHC", "SLC","SLMC","SMC","SMHC","SHC","FSL","L","ZL","FSCL","FSLC","FSLMC","FSMC","FSMHC"
            ,"FSHC") OR
            A.TEXTURE_CODE IN ( "L","ZL","SCL","LFSY","FSCL","KSCL") and
            B.TEXTURE_CODE in ( "MHC","HC","MC","LMC","KSMHC","KSHC","FSMHC","FSHC","KSMC","KSLMC","FSMC",
            "FSLMC") OR
            A.TEXTURE_CODE IN ( "CL","CLS","CLFS","CLKS") and
            B.TEXTURE_CODE in ( "MHC","HC","MC","KSMHC","KSHC","FSMHC","FSHC","KSMC","FSMC")
              ) OR (
              B.VALUE_2Z2_Clay >= 2*A.VALUE_2Z2_Clay
              ))') 


 ku_1 <- anti_join(ku_1,ku_1a,by = c('PROJECT_CODE','SITE_ID'))  %>%  distinct(PROJECT_CODE,SITE_ID)


ku_2 <- ku_nnull[which(grepl("B2",ku_nnull$HORIZON_NAME) & (ku_nnull$VALUE_4A1 < 5.5 |ku_nnull$FTS_PH <5.5)),]
ku_2 <- ku_2 %>%  distinct(PROJECT_CODE,SITE_ID)
#combine ku_1 and ku_2
ku_p <- inner_join(ku_1,ku_2)

#add a column to or
ku_p$ASC_ORD <- rep("KU",nrow(ku_p))
names(ku_p)[3] <- 'ASC_ORD_P'
all_p <- full_join(all_p,ku_p)


# Confusion matrix

# dataset with null value in attributes for judging OR, and is labeled with OR
# ku_null <- filter(ocl[,c(1,2,7)], ASC_ORD == 'KU') %>%
#   anti_join(.,ku_nnull,by = c('PROJECT_CODE','SITE_ID')) %>%
#   left_join(.,sit[,c('PROJECT_CODE','SITE_ID',"ELEM_TYPE_CODE")]) %>%
#   left_join(.,obs[,c('PROJECT_CODE','SITE_ID','DRAINAGE')],
#             by = c('PROJECT_CODE','SITE_ID') ) 

# ku_nnull distinct with project_code and site_id, and add asc_ord from ocl
ku_vat <- ku_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
# unique check
nrow(unique(ku_vat[,1:2])) == nrow(unique(ku_vat[,1:3]))
#join the predict data
ku_vat <- left_join(ku_vat,ku_p,by = c('PROJECT_CODE','SITE_ID'))
# mark other orders except "ku" in ASC_ORD with "OTHERS",
ku_vat[which(!is.na(ku_vat$ASC_ORD) & ku_vat$ASC_ORD != 'KU'),'ASC_ORD'] = 'OTHERS'
# NA in asc_ord as "N/A"
ku_vat[is.na(ku_vat)]<-"N/A"
# value in ASC_ORD_P except for 'ku', and is "OTHERS" in ASC_ORD, mark as "OTHERS" as well
ku_vat[which(ku_vat$ASC_ORD == "OTHERS" & ku_vat$ASC_ORD_P != 'KU'),'ASC_ORD_P'] = 'OTHERS'
# feed the data to confusionmatrix
ku_confmat <- confusionMatrix(as.factor(ku_vat$ASC_ORD_P),reference = as.factor(ku_vat$ASC_ORD))
print(ku_confmat)

# similar as aboku but tick out the "N/A" values in ASC_ORD
ku_vat_1 <- ku_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
ku_vat_1 <- ku_vat_1[which(!is.na(ku_vat_1$ASC_ORD)),]
nrow(unique(ku_vat_1[,1:2])) == nrow(unique(ku_vat_1[,1:3]))
ku_vat_1 <- left_join(ku_vat_1,ku_p,by = c('PROJECT_CODE','SITE_ID'))
ku_vat_1[which(ku_vat_1$ASC_ORD != 'KU'),'ASC_ORD'] = 'OTHERS'
ku_vat_1[is.na(ku_vat_1)]<-"OTHERS"
ku_confmat_1 <- confusionMatrix(as.factor(ku_vat_1$ASC_ORD_P),reference = as.factor(ku_vat_1$ASC_ORD))
print(ku_confmat_1)



# write to files
ku_tp <- filter(ku_vat_1,ASC_ORD == 'KU',ASC_ORD_P == 'KU') 
ku_tp_hor <- left_join(ku_tp,ku_nnull, by = c('PROJECT_CODE','SITE_ID') )
ku_fp_fn <- filter(ku_vat_1,ASC_ORD != ASC_ORD_P) 
ku_fp_fn <- sqldf('select A.PROJECT_CODE, A.SITE_ID, B.ASC_ORD, A.ASC_ORD_P
        FROM ku_fp_fn A, ocl B
        WHERE A.PROJECT_CODE = B.PROJECT_CODE AND
        A.SITE_ID = B.SITE_ID')
ku_fp_fn[which(is.na(ku_fp_fn)),]  

ku_fp_fn_hor <- left_join(ku_fp_fn,ku_nnull)


# ku_fp_fn_hor$ku_1 <- ifelse(is.na(match(paste0(ku_fp_fn_hor$PROJECT_CODE,ku_fp_fn_hor$SITE_ID),
#                                          paste0(ku_1$PROJECT_CODE,ku_1$SITE_ID))),"NO","YES")
# ku_fp_fn_hor$ku_p <- ifelse(is.na(match(paste0(ku_fp_fn_hor$PROJECT_CODE,ku_fp_fn_hor$SITE_ID),
#                                          paste0(ku_p$PROJECT_CODE,ku_p$SITE_ID))),"NO","YES")

# ku_fp_fn_hor[which(is.na(ku_fp_fn_hor)),]

ku_predict <- filter(ku_vat,ASC_ORD =="N/A",ASC_ORD_P == 'KU' )

write.csv(ku_fp_fn, file =paste("C:/Users/lzccn/Downloads/des/files/ku_fp_fn",
                                format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
write.csv(ku_fp_fn_hor, file =paste("C:/Users/lzccn/Downloads/des/files/ku_fp_fn_hor",
                                    format(Sys.time(),"%d-%b-%Y %H.%M"),".csv",sep = " "))
write.csv(ku_predict, file =paste("C:/Users/lzccn/Downloads/des/files/ku_predict",
                                  format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
# write.csv(ku_null, file =paste("C:/Users/lzccn/Downloads/des/files/ku_null",
#                                format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
...
so_nnull <- hor[,c('PROJECT_CODE','SITE_ID','HORIZON_NO',
                   'HORIZON_NAME')] %>%
  left_join(.,lab_15n1,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO')) %>%
  # Delete data with NA_ESP value in B2 horizon
  right_join(.,.[which(!is.na(.$VALUE_15N1) & grepl("B2",.$HORIZON_NAME)),],) %>% 
  distinct(PROJECT_CODE,SITE_ID) %>%
  left_join(.,hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
         'HORIZON_NAME','TEXTURE_CODE','BOUND_DISTINCT')],by = c('PROJECT_CODE','SITE_ID')) %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  left_join(.,lab_15n1,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO')) %>%
  anti_join(.,filter(all_p,ASC_ORD_P !='SO' ),by= c('PROJECT_CODE','SITE_ID'))%>%
  filter(.,!ASC_ORD %in% c('AN','OR','PO','VE','HY','KU'))

 #contains B2, ESP>=6
so_1 <- so_nnull[which( grepl("B2",so_nnull$HORIZON_NAME) & so_nnull$VALUE_15N1 >=6),]
...
#add a column to or
so_p$ASC_ORD <- rep("SO",nrow(so_p))

names(so_p)[9] <- 'ASC_ORD_P'
all_p <- full_join(all_p,so_p)


# Confusion matrix

# dataset with null value in attributes for judging OR, and is labeled with OR
# so_null <- filter(ocl[,c(1,2,7)], ASC_ORD == 'SO') %>%
#   anti_join(.,so_nnull,by = c('PROJECT_CODE','SITE_ID')) %>%
#   left_join(.,sit[,c('PROJECT_CODE','SITE_ID',"ELEM_TYPE_CODE")]) %>%
#   left_join(.,obs[,c('PROJECT_CODE','SITE_ID','DRAINAGE')],
#             by = c('PROJECT_CODE','SITE_ID') ) 

# so_nnull distinct with project_code and site_id, and add asc_ord from ocl
so_vat <- so_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
# unique check
nrow(unique(so_vat[,1:2])) == nrow(unique(so_vat[,1:3]))
#join the predict data
so_vat <- left_join(so_vat,so_p,by = c('PROJECT_CODE','SITE_ID'))
# mark other orders except "so" in ASC_ORD with "OTHERS",
so_vat[which(!is.na(so_vat$ASC_ORD) & so_vat$ASC_ORD != 'SO'),'ASC_ORD'] = 'OTHERS'
# NA in asc_ord as "N/A"
so_vat[is.na(so_vat)]<-"N/A"
# value in ASC_ORD_P except for 'so', and is "OTHERS" in ASC_ORD, mark as "OTHERS" as well
so_vat[which(so_vat$ASC_ORD == "OTHERS" & so_vat$ASC_ORD_P != 'SO'),'ASC_ORD_P'] = 'OTHERS'
# feed the data to confusionmatrix
so_confmat <- confusionMatrix(as.factor(so_vat$ASC_ORD_P),reference = as.factor(so_vat$ASC_ORD))
print(so_confmat)

# similar as aboso but tick out the "N/A" values in ASC_ORD
so_vat_1 <- so_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
so_vat_1 <- so_vat_1[which(!is.na(so_vat_1$ASC_ORD)),]
nrow(unique(so_vat_1[,1:2])) == nrow(unique(so_vat_1[,1:3]))
so_vat_1 <- left_join(so_vat_1,so_p,by = c('PROJECT_CODE','SITE_ID'))
so_vat_1[which(so_vat_1$ASC_ORD != 'SO'),'ASC_ORD'] = 'OTHERS'
so_vat_1[is.na(so_vat_1)]<-"OTHERS"
so_confmat_1 <- confusionMatrix(as.factor(so_vat_1$ASC_ORD_P),reference = as.factor(so_vat_1$ASC_ORD))
print(so_confmat_1)



# write to files
so_tp <- filter(so_vat_1,ASC_ORD == 'SO',ASC_ORD_P == 'SO') 
so_tp_hor <- left_join(so_tp,so_nnull, by = c('PROJECT_CODE','SITE_ID') )
so_fp_fn <- filter(so_vat_1,ASC_ORD != ASC_ORD_P) 
so_fp_fn <- sqldf('select A.PROJECT_CODE, A.SITE_ID, B.ASC_ORD, A.ASC_ORD_P
        FROM so_fp_fn A, ocl B
        WHERE A.PROJECT_CODE = B.PROJECT_CODE AND
        A.SITE_ID = B.SITE_ID')
so_fp_fn[which(is.na(so_fp_fn)),]  

so_fp_fn_hor <- left_join(so_fp_fn,so_nnull)


# so_fp_fn_hor$so_1 <- ifelse(is.na(match(paste0(so_fp_fn_hor$PROJECT_CODE,so_fp_fn_hor$SITE_ID),
#                                          paste0(so_1$PROJECT_CODE,so_1$SITE_ID))),"NO","YES")
# so_fp_fn_hor$so_p <- ifelse(is.na(match(paste0(so_fp_fn_hor$PROJECT_CODE,so_fp_fn_hor$SITE_ID),
#                                          paste0(so_p$PROJECT_CODE,so_p$SITE_ID))),"NO","YES")

# so_fp_fn_hor[which(is.na(so_fp_fn_hor)),]

so_predict <- filter(so_vat,ASC_ORD =="N/A",ASC_ORD_P == 'SO' )

write.csv(so_fp_fn, file =paste("C:/Users/lzccn/Downloads/des/files/so_fp_fn",
                                format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
write.csv(so_fp_fn_hor, file =paste("C:/Users/lzccn/Downloads/des/files/so_fp_fn_hor",
                                    format(Sys.time(),"%d-%b-%Y %H.%M"),".csv",sep = " "))
write.csv(so_predict, file =paste("C:/Users/lzccn/Downloads/des/files/so_predict",
                                  format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
# write.csv(so_null, file =paste("C:/Users/lzccn/Downloads/des/files/so_null",
#                                format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
...
ch_nnull <- hor[,c('PROJECT_CODE','SITE_ID','OBS_NO','HORIZON_NO','DESIGN_MASTER',
                   'HORIZON_NAME','TEXTURE_CODE','BOUND_DISTINCT')] %>%
  left_join(.,ocl[,c('PROJECT_CODE','SITE_ID','ASC_ORD')]) %>%
  left_join(.,fts,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO')) %>%
  left_join(.,lab_4a1,by = c('PROJECT_CODE','SITE_ID','HORIZON_NO')) %>%
  ...
  by = c('PROJECT_CODE','SITE_ID')) %>%
  anti_join(.,filter(all_p,ASC_ORD_P !='CH' ),by= c('PROJECT_CODE','SITE_ID'))%>%
  filter(.,!ASC_ORD %in% c('AN','OR','PO','VE','HY','KU','SO'))

ch_2 <- ch_nnull[which(grepl("B2",ch_nnull$HORIZON_NAME) & (ch_nnull$VALUE_4A1 >=5.5 |ch_nnull$FTS_PH >= 5.5)),]
ch_2 <- ch_2 %>%  distinct(PROJECT_CODE,SITE_ID)

ch_p <-anti_join(ku_1,ku_p) %>% anti_join(.,so_p) %>% inner_join(.,ch_2)
...
all_p <- full_join(all_p,ch_p)


# Confusion matrix

# dataset with null value in attributes for judging OR, and is labeled with OR
# ch_null <- filter(ocl[,c(1,2,7)], ASC_ORD == 'CH') %>%
#   anti_join(.,ch_nnull,by = c('PROJECT_CODE','SITE_ID')) %>%
#   left_join(.,sit[,c('PROJECT_CODE','SITE_ID',"ELEM_TYPE_CODE")]) %>%
#   left_join(.,obs[,c('PROJECT_CODE','SITE_ID','DRAINAGE')],
#             by = c('PROJECT_CODE','SITE_ID') ) 

# ch_nnull distinct with project_code and site_id, and add asc_ord from ocl
ch_vat <- ch_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
# unique check
nrow(unique(ch_vat[,1:2])) == nrow(unique(ch_vat[,1:3]))
#join the predict data
ch_vat <- left_join(ch_vat,ch_p,by = c('PROJECT_CODE','SITE_ID'))
# mark other orders except "ch" in ASC_ORD with "OTHERS",
ch_vat[which(!is.na(ch_vat$ASC_ORD) & ch_vat$ASC_ORD != 'CH'),'ASC_ORD'] = 'OTHERS'
# NA in asc_ord as "N/A"
ch_vat[is.na(ch_vat)]<-"N/A"
# value in ASC_ORD_P except for 'ch', and is "OTHERS" in ASC_ORD, mark as "OTHERS" as well
ch_vat[which(ch_vat$ASC_ORD == "OTHERS" & ch_vat$ASC_ORD_P != 'CH'),'ASC_ORD_P'] = 'OTHERS'
# feed the data to confusionmatrix
ch_confmat <- confusionMatrix(as.factor(ch_vat$ASC_ORD_P),reference = as.factor(ch_vat$ASC_ORD))
print(ch_confmat)

# similar as aboch but tick out the "N/A" values in ASC_ORD
ch_vat_1 <- ch_nnull %>%
  distinct(.,PROJECT_CODE,SITE_ID) %>%
  left_join(.,ocl[,c(1,2,7)],by = c('PROJECT_CODE','SITE_ID')) 
ch_vat_1 <- ch_vat_1[which(!is.na(ch_vat_1$ASC_ORD)),]
nrow(unique(ch_vat_1[,1:2])) == nrow(unique(ch_vat_1[,1:3]))
ch_vat_1 <- left_join(ch_vat_1,ch_p,by = c('PROJECT_CODE','SITE_ID'))
ch_vat_1[which(ch_vat_1$ASC_ORD != 'CH'),'ASC_ORD'] = 'OTHERS'
ch_vat_1[is.na(ch_vat_1)]<-"OTHERS"
ch_confmat_1 <- confusionMatrix(as.factor(ch_vat_1$ASC_ORD_P),reference = as.factor(ch_vat_1$ASC_ORD))
print(ch_confmat_1)



# write to files
ch_tp <- filter(ch_vat_1,ASC_ORD == 'CH',ASC_ORD_P == 'CH') 
ch_tp_hor <- left_join(ch_tp,ch_nnull, by = c('PROJECT_CODE','SITE_ID') )
ch_fp_fn <- filter(ch_vat_1,ASC_ORD != ASC_ORD_P) 
ch_fp_fn <- sqldf('select A.PROJECT_CODE, A.SITE_ID, B.ASC_ORD, A.ASC_ORD_P
        FROM ch_fp_fn A, ocl B
        WHERE A.PROJECT_CODE = B.PROJECT_CODE AND
        A.SITE_ID = B.SITE_ID')
ch_fp_fn[which(is.na(ch_fp_fn)),]  

ch_fp_fn_hor <- left_join(ch_fp_fn,ch_nnull)


# ch_fp_fn_hor$ch_1 <- ifelse(is.na(match(paste0(ch_fp_fn_hor$PROJECT_CODE,ch_fp_fn_hor$SITE_ID),
#                                          paste0(ch_1$PROJECT_CODE,ch_1$SITE_ID))),"NO","YES")
# ch_fp_fn_hor$ch_p <- ifelse(is.na(match(paste0(ch_fp_fn_hor$PROJECT_CODE,ch_fp_fn_hor$SITE_ID),
#                                          paste0(ch_p$PROJECT_CODE,ch_p$SITE_ID))),"NO","YES")

# ch_fp_fn_hor[which(is.na(ch_fp_fn_hor)),]

ch_predict <- filter(ch_vat,ASC_ORD =="N/A",ASC_ORD_P == 'CH' )

write.csv(ch_fp_fn, file =paste("C:/Users/lzccn/Downloads/des/files/ch_fp_fn",
                                format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
write.csv(ch_fp_fn_hor, file =paste("C:/Users/lzccn/Downloads/des/files/ch_fp_fn_hor",
                                    format(Sys.time(),"%d-%b-%Y %H.%M"),".csv",sep = " "))
write.csv(ch_predict, file =paste("C:/Users/lzccn/Downloads/des/files/ch_predict",
                                  format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
# write.csv(ch_null, file =paste("C:/Users/lzccn/Downloads/des/files/ch_null",
#                                format(Sys.time(),"%d-%b-%Y %H.%M"), ".csv",sep = " "))
